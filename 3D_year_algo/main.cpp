...
int main() {
  const size_t kRankOfBTree = 10;
  BTree<kRankOfBTree>* tree = new BTree<kRankOfBTree>;
  std::string interm;
  int item = 0;
  while (std::cin >> interm) {
    if (interm == "insert") {
      std::cin >> item;
      tree->Insert(item);
    } else if (interm == "delete") {
      std::cin >> item;
      tree->Delete(item);
    } else if (interm == "exists") {
      std::cin >> item;
      if (tree->Exists(item)) {
        std::cout << "true\n";
      } else {
